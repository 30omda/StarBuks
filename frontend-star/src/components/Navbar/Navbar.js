...
import "./Navbar.css";
import location from "../../assets/img/location-svgrepo-com.svg";
import $ from "jquery";
...
let rewards    = "Starbucks® Rewards -- Starbucks® Rewards --  Starbucks® Rewards --  Starbucks® Rewards --  Starbucks® Rewards --  Starbucks® Rewards --  Starbucks® Rewards  ";
let menu       = "Starbucks® Menu -- Starbucks® Menu --  Starbucks® Menu --  Starbucks® Menu --  Starbucks® Menu --  Starbucks® Menu --  Starbucks® Menu --  Starbucks® Menu   ";
let gift       = "Starbucks® Gift Cards -- Starbucks® Gift Cards --  Starbucks® Gift Cards --  Starbucks® Gift Cards --  Starbucks® Gift Cards --  Starbucks® Gift Cards --  Starbucks® Gift Cards  ";
let joinUs     = "Starbucks® Join Us -- Starbucks® Join Us --  Starbucks® Join Us --  Starbucks® Join Us --  Starbucks® Join Us --  Starbucks® Join Us --  Starbucks® Join Us --  Starbucks® Join Us  ";
let locations  = "Starbucks® location -- Starbucks® location --  Starbucks® location --  Starbucks® location --  Starbucks® location --  Starbucks® location --  Starbucks® location  ";

const [words , setWords] = useState(rewards)
...
useEffect(()=>{
  if (navigates.pathname.includes("/giftcards")){
    $(`.giftcard`).addClass('active').siblings().removeClass('active')
    setWords(gift);
  }else if (navigates.pathname == "/"){
    $(`.reward`).addClass('active').siblings().removeClass('active')
    setWords(rewards);
  }else if (navigates.pathname.includes("/menu")){
    $(`.menu`).addClass('active').siblings().removeClass('active')
    setWords(menu);
    setNavActive(true);
  }else if(navigates.pathname.includes("/login")){
setWords(joinUs);
$(`.menu`).removeClass('active').siblings().removeClass('active')
  }else if(navigates.pathname.includes("/findstore")){
    setWords(locations);
    $(`.menu`).removeClass('active').siblings().removeClass('active')
      } else if (navigates.pathname == "/cart"){
    setNavActive(false);
  }else{
      setNavActive(true)
    } 
  ...
  return (
    <nav>
      ...
      <div className="d-flex "> 
        ...
        <div className="nav-menu d-flex w-100  justify-content-between  ">
        <ul className="navlist1 d-flex">
          <li className="navlist text-upper menu"     onClick={()=>{setWords(menu);setActive("/menu")}}>              <Link to = "/menu"       className="lists" >MENU</Link >       </li>
          <li className="navlist text-upper reward"   onClick={()=>{setWords(rewards);setActive("/")}}>               <Link to = "/"           className="lists" >REWARDS</Link >    </li>
          <li className="navlist text-upper giftcard" onClick={()=>{setWords(gift);setActive("/giftcards")}}>         <Link to = "/giftcards"  className="lists" >GIFT CARDS</Link > </li>
        </ul>
      


      <div>
        <ul className="navlist2 d-flex">
    
          <li>
          {navigates.pathname.includes("/findstore")?console.log("none"):<Link to="/findstore"><img className="mx-2 d-inline" src={location} alt="location" />Find a store</Link>}
          </li>
        
          {user == undefined ?   <li>
            {navigates.pathname.includes("/login")?console.log("none")  :<Link to="/login">Join now</Link>}
          </li> : (<li > <p > {user.userName} </p></li>)} 

          {user == undefined ? "" : ( <li className="logout">
                       ...
                       render={(renderProps) => (
                           ...
                           <button
                           type="button"
                           className=" bg-white p-2 rounded-full cursor-pointer outline-none shadow-md sm:relative lg:bottom-2  block  "
    ...
    <div className="hamburger" onClick={hambClick} >
      ...
      <div className="down">
        <h6 className="text-upper">{words}</h6>
